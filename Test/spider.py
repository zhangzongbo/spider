...
def get_hot_comments(song_id, threshold=COMMENT_THRESHOLD):
    ...
    """
    url = BASE_URL + \
        'weapi/v1/resource/comments/R_SO_4_{}?csrf_token='.format(song_id)
    headers = {'Cookie': 'appver=1.5.0.75771;',
...
def get_comments_by_api(song_id, threshold=COMMENT_THRESHOLD):
    """
    这个接口不需要加密
    """
    url = BASE_URL + \
...
if __name__ == "__main__":
    ...
    song_list = get_song_list(13193)
    print(song_list)
    for song in song_list:
        print u'歌曲： {} 的热门评论有： '.format(song[0])
        hot_comments = get_comments_by_api(song[1][9:])
